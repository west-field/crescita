...
public class armorPanelScript : MonoBehaviour
{
    //アイテム情報のスロットのプレハブ
    ...
    [SerializeField] private AudioClip armorSound, moveSound,levelUpSound,noSound;//開くときの音、カーソルの音
    ...
    private void Update()
    {
        ...
        if (Submit.WasPressedThisFrame())
        {
            ...
            foreach (var item in materialName)
            {
                ...
                Debug.Log($"防具屋　{item.name}を{item.count}持っている");
            ...
            if (isHaveMaterial)
            {
                ...
                //必要な素材がそろっている時防具レベルを上げる
                playerStartas.ArmorLevelUp();
                isLevelUp = true;
    ...
    private void CreateSlot()
    {
        ...
        materialName.Clear();
        Material material;
        switch (playerStartas.GetArmorPower())
        {
            case 1:
